    ...
    <div class="content">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="180px">
        <el-form-item  label="商家账号" prop="businessPhone">
          <el-col :span="12">
            <el-input v-model="ruleForm.businessPhone" placeholder="请输入商家账号"></el-input>
          </el-col>
        </el-form-item>
    ...
    methods: {
      handleAdd() {
        ...
        var param = Object.assign({}, {userPhone: user.userPhone , token: user.token ,
          needAddPhone: this.ruleForm.businessPhone , shopTotal: this.ruleForm.shopTotal});

        //查询菜单列表
        API.POST(URL.ADD_USER_URL, param)
          .then(res => {
            if (res.result.retCode === 0) {
              this.$router.go(-1);
            }if (res.result.retCode === 1001) {
              this.$message.error('商家已存在！');
            }
          })
          .catch(err => {
            console.log(err);
